%% Fast Sinkhorn Filters - Function to convert a functional map to a pointwise correspondence
%{
***Input***
...
Main script written by Gautam Pai, March 2021.

%}
%%
function [S,T12,T21] = fast_sinkhorn_filter(KTar,KSrc,options)
%% Default Parameters
...
%% Kernel Construction
    ...
    switch options.kernel_type
        ...
        case 'full' % Populate Full Kernel Matrix

            Ds = pdist2(KSrc,KTar).^options.p; 
            lambda = 200/(max(max(Ds)));
            K = exp(-Ds*lambda); 

        case 'sparse' % Populate a sparse Kernel

            knn = options.knn;
            distmax = options.distmax;
            
            [J,Dist] = knnsearch(KSrc,KTar,'K',knn,'Distance','Euclidean','NSMethod','kdtree');
            lambda = distmax/(max(max(Dist)));
            
            Kdist = exp(-Dist.^options.p*lambda);
            [~,I] = meshgrid(1:knn,1:size(KTar,1));
            K1 = sparse(I(:),J(:),Kdist(:),size(KTar,1),size(KSrc,1));

            [J,Dist] = knnsearch(KTar,KSrc,'K',knn,'Distance','Euclidean','NSMethod','kdtree');
            lambda = distmax/(max(max(Dist)));
            Kdist = exp(-Dist.^options.p*lambda);
            [~,I] = meshgrid(1:knn,1:size(KSrc,1));
            K2 = sparse(I(:),J(:),Kdist(:),size(KSrc,1),size(KTar,1));

            K = (K1'+K2)/2; % Kernel populated symmetrically for better results

    end
        
%% Sinkhorn Iterations

a  = ones(size(KSrc,1),1);a = a/sum(a); % uniform distribution on Target
b  = ones(size(KTar,1),1);b = b/sum(b); % uniform distribution on Source

if options.maxiter>0
    [~,~,u,v]=sinkhornTransport(a,b,K,[],lambda,[],[],[],options.maxiter,0); % Sinkhorn iterations
    S = u.*(K.*v'); % construct doubly stochastic matrix
else
    S = K; % This just gives the nearest neighbor solution
end

[~,T12] = max(S,[],2); % extract pointwise forward map
[~,T21] = max(S,[],1); % extract pointwise backward map
T21 = T21';

end

%% Code from Marco Cuturi for Sinkhorn Iterations

% Please refer : http://marcocuturi.net/SI.html for all the details.
